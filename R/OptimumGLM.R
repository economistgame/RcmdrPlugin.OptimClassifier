...
#' @name OptimumGLM
#'
#' @description This dialog is used to specify a generalized linear model to be fit by the \link[OptimClassifier]{Optim.GLM} function.
#'
...
OptimumGLM <- function(){
  ##Windows Title
  initializeDialog(title=gettextRcmdr("OptimGLM"))
  ###Function to calc last number
  UpdateModelNumber()
  ## Create a Model Name
  modelName <- tclVar(paste("OptimGLM", getRcmdr("modelNumber"), sep=""))
  ...
  onOK <- function(){ # Actions to perform
    ...
    if (0 == length(y)) {
      UpdateModelNumber(-1)
      errorCondition(recall=OptimumGLM, message=gettextRcmdr("You must select a response variable."))
      return()
    }
    if (0 == length(x)) {
      UpdateModelNumber(-1)
      errorCondition(recall=OptimumGLM, message=gettextRcmdr("No explanatory variables selected."))
      return()
    }
    if (is.element(y, x)) {
      UpdateModelNumber(-1)
      errorCondition(recall=OptimumGLM, message=gettextRcmdr("Response and explanatory variables must be different."))
      return()
    }
    modelValue <- trim.blanks(tclvalue(modelName))
    if (!is.valid.name(modelValue)){
      UpdateModelNumber(-1)
      errorCondition(recall=OptimumGLM, message=sprintf(gettextRcmdr('"%s" is not a valid name.'), modelValue))
      return()
    }
    ...
    command <- paste("Optim.GLM(", y, "~", paste(x, collapse="+"),
  ...
  # Set up GUI
  OKCancelHelp(helpSubject="Optim.GLM")
