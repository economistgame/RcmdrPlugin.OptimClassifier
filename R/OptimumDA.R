...
#' @name OptimumDA
#'
#' @description This dialog is used to specify a  discriminant analysis model to be fit by the \link[OptimClassifier]{Optim.DA} function.
#'
#' @seealso \code{\link[OptimClassifier]{Optim.DA}}
#'
#' @examples{
#' # Examples of use are provided in the help of each option of menu.
#' }
#'
#' @import Rcmdr RcmdrMisc OptimClassifier
...
OptimumDA <- function(){
  ##Windows Title
  initializeDialog(title=gettextRcmdr("OptimDA Model"))
  ###Function to calc last number
  UpdateModelNumber()
  ## Create a Model Name
  modelName <- tclVar(paste("OptimDA", getRcmdr("modelNumber"), sep=""))
  ...
  onOK <- function(){ # Actions to perform
    ...
    if (0 == length(y)) {
      UpdateModelNumber(-1)
      errorCondition(recall=OptimumDA, message=gettextRcmdr("You must select a response variable."))
      return()
    }
    if (0 == length(x)) {
      UpdateModelNumber(-1)
      errorCondition(recall=OptimumDA, message=gettextRcmdr("No explanatory variables selected."))
      return()
    }
    if (is.element(y, x)) {
      UpdateModelNumber(-1)
      errorCondition(recall=OptimumDA, message=gettextRcmdr("Response and explanatory variables must be different."))
      return()
    }
    modelValue <- trim.blanks(tclvalue(modelName))
    if (!is.valid.name(modelValue)){
      UpdateModelNumber(-1)
      errorCondition(recall=OptimumDA, message=sprintf(gettextRcmdr('"%s" is not a valid name.'), modelValue))
      return()
    }
    ...
    command <- paste("Optim.DA(", y, "~", paste(x, collapse="+"),
  ...
  # Set up GUI
  OKCancelHelp(helpSubject="Optim.DA")
  ...
  radioButtons(name="OrderRank", buttons=c("RMSE","SuccessRate", "TIerror","TIIerror"), values=c("rmse","success", "ti_error","tii_error"),
               labels=gettextRcmdr(c("RMSE","Success Rate", "TI error","TII error")), title=gettextRcmdr("Criteria"))
