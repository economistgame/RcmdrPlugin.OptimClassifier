...
#' @name OptimumLM
#'
#' @description This dialog is used to specify a  linear model to be fit by the \link[OptimClassifier]{Optim.LM} function.
#'
#' @seealso \code{\link[Rcmdr]{linearModel}}, \code{\link[OptimClassifier]{Optim.LM}}
#'
...
OptimumLM <- function(){
  ##Windows Title
  initializeDialog(title=gettextRcmdr("OptimLM"))
  ###Function to calc last number
  UpdateModelNumber()
  ## Create a Model Name
  modelName <- tclVar(paste("OptimLM", getRcmdr("modelNumber"), sep=""))
  ...
  onOK <- function(){ # Actions to perform
    ...
    if (0 == length(y)) {
      UpdateModelNumber(-1)
      errorCondition(recall=OptimumLM, message=gettextRcmdr("You must select a response variable."))
      return()
    }
    if (0 == length(x)) {
      UpdateModelNumber(-1)
      errorCondition(recall=OptimumLM, message=gettextRcmdr("No explanatory variables selected."))
      return()
    }
    if (is.element(y, x)) {
      UpdateModelNumber(-1)
      errorCondition(recall=OptimumLM, message=gettextRcmdr("Response and explanatory variables must be different."))
      return()
    }
    modelValue <- trim.blanks(tclvalue(modelName))
    if (!is.valid.name(modelValue)){
      UpdateModelNumber(-1)
      errorCondition(recall=OptimumLM, message=sprintf(gettextRcmdr('"%s" is not a valid name.'), modelValue))
      return()
    }
    ...
    command <- paste("Optim.LM(", y, "~", paste(x, collapse="+"),
  ...
  # Set up GUI
  OKCancelHelp(helpSubject="Optim.LM")
